...
If you have SSH access to the server, you can deploy directly to the plugin volume. Substitute the latest version tag from
[GitHub Releases](https://github.com/Such-Software/btcpayserver-grin-plugin/releases) for `<VERSION>` below:
...
mkdir -p "$PLUGIN_DIR"
python3 -c "import zipfile; zipfile.ZipFile('/tmp/${VERSION}.0/BTCPayServer.Plugins.Grin.btcpay').extractall('$PLUGIN_DIR')"
...
7. Reload the page — the **Wallet Balance** card appears under the
   sync-status banner, showing spendable / awaiting confirmation /
   locked / total GRIN at a glance.

### Accept your first payment (end-to-end smoke test)
...
- **Wallet credentials** (password, API secret, webhook secret) are
  stored in BTCPay's PostgreSQL database, **encrypted at rest** via
  ASP.NET Core's `IDataProtector` (v1.0.10+). The decryption key
  lives in BTCPay's DataProtection key ring — back up that directory
  alongside your Postgres dumps. See [SECURITY.md](SECURITY.md) for
  full details + key-ring durability notes.
- **The Owner API uses encrypted RPC** — even over HTTP, payloads are
  AES-256-GCM encrypted (ECDH key exchange + AES-256-GCM)
- **The plugin never holds private keys** — all signing happens in
  grin-wallet
- **Bind the Owner API to localhost** (`127.0.0.1`) and use socat for
  Docker access
- **Don't expose port 3420 to the internet** — the API has full
  wallet control
- **Self-hosted BTCPay = self-custodial**. Third-party BTCPay = you're
  trusting the operator with wallet credentials (same trust model as
  Lightning in BTCPay)
